...
public class PlayerWeaponManager : NetworkBehaviour
{
    ...
    {
        if (!NetworkManager.SpawnManager.SpawnedObjects.TryGetValue(pickupObjectId, out NetworkObject pickupObject))
        {
            return;
        }
    ...
    public void RequestShoot(Vector3 origin, Vector3 direction, Vector3 muzzlePosition)
    {
    ...
    [Rpc(SendTo.Server)]
    private void ShootRpc(Vector3 origin, Vector3 direction, Vector3 muzzlePosition)
    {
        ...
        Vector3 hitPoint = origin + direction * gunData.range;
        ...
        if (Physics.Raycast( origin, direction, out RaycastHit hit, gunData.range))
        {
                ...
                !targetHealth.IsDead)
            {
                ...
                SpawnFloatingDamageRpc(hit.point + Vector3.up, damageAmount);
                if (killed && GameManager.main != null)
                    GameManager.main.AddPoint(OwnerClientId);
            }
        }

        PlayShootFeedbackRpc();
    ...
    private void ReloadRpc()
    {
        ...
        PlayReloadFeedbackRpc();
    }

    [Rpc(SendTo.Everyone)]
    private void PlayShootFeedbackRpc()
    {
        currentGun?.PlayShootFeedback();
    }

    [Rpc(SendTo.Everyone)]
    private void PlayReloadFeedbackRpc()
    {
        currentGun?.PlayReloadFeedback();
    }
        ...
        Vector3 hitPoint)
    {
        ...
        GameObject bullet = Instantiate( currentGun.BulletPrefab, startPosition, Quaternion.LookRotation(direction));

        BulletVisual bulletVisuals = bullet.GetComponent<BulletVisual>();
    ...
    [Rpc(SendTo.Everyone)]
    private void SpawnFloatingDamageRpc(Vector3 position, int damageAmount)
    {
        if (currentGun == null || currentGun.FloatingDamagePrefab == null)
        {
            return;
        }

        FloatingDamage floatingDamage = Instantiate(currentGun.FloatingDamagePrefab, position, Quaternion.identity);
    ...
    private void EquipWeapon(int weaponId)
    {
        ...
        currentGun = Instantiate(weaponPrefab, weaponHolder);
